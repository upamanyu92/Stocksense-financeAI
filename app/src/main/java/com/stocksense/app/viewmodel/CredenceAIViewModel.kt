...
class CredenceAIViewModel(
    ...
    init {
        observeHistory()
        // Reactively observe LLM status — propagates LOADING → READY → LOAD_FAILED to UI.
        viewModelScope.launch {
            llmEngine.statusFlow.collect { s ->
                _uiState.update { it.copy(llmStatus = s, llmQualityMode = llmEngine.currentQualityMode()) }
            }
        }
    }
